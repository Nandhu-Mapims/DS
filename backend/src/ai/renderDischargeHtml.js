 ...
 * Matches device_bluetheme.html / device_without_blue.html layout and styling.
 * Safe escaping for all text fields.
 */
...
export function renderDischargeHtml(json) {
  ...
  if (json.finalNarrativeText && String(json.finalNarrativeText).trim()) {
    html += '<section><div class="section-title">Clinical Summary</div><div class="content-text">';
    html += para(json.finalNarrativeText);
    html += '</div></section>';
  }

  if (json.investigations && String(json.investigations).trim()) {
    html += '<section class="new-page-section"><div class="section-title">Laboratory Investigations</div><div class="content-text">';
    html += para(json.investigations);
    html += '</div></section>';
  }

  if (d.final || d.provisional || (d.icd10Codes && d.icd10Codes.length)) {
    html += '<section><div class="section-title">Final Diagnosis</div><div class="diagnosis-box">';
    if (d.final) html += `<p><strong>Primary Diagnosis:</strong> ${escapeHtml(d.final)}.</p>`;
    if (d.provisional && d.provisional !== d.final) html += `<p style="margin-top: 10px; font-weight: bold; color: var(--primary-color);">Secondary Diagnoses:</p><ul><li>${escapeHtml(d.provisional)}</li></ul>`;
    else if (d.provisional) html += `<p><strong>Provisional:</strong> ${escapeHtml(d.provisional)}.</p>`;
    if (d.icd10Codes && d.icd10Codes.length) html += `<p><strong>ICD-10:</strong> ${escapeHtml(d.icd10Codes.join(', '))}</p>`;
    html += '</div></section>';
  }

  if (json.hospitalCourse && String(json.hospitalCourse).trim()) {
    html += '<section class="new-page-section"><div class="section-title">Hospital Course &amp; Care Provided</div><div class="content-text">';
    html += para(json.hospitalCourse);
    html += '</div></section>';
  }

  if (json.procedures && String(json.procedures).trim()) {
    html += '<section><div class="section-title">Procedures Performed</div><div class="content-text">';
    html += para(json.procedures);
    html += '</div></section>';
  }
  ...
  if (hasPostDischarge) {
    ...
    if (Array.isArray(meds) && meds.length > 0) {
      html += '<p style="font-weight: bold; margin-bottom: 8px;">Discharge Medications</p>';
  ...
  html += '<div class="signature-area"><div class="sig-box"><div class="sig-line"></div><p>Patient / Attendant Signature</p></div><div class="sig-box"><div class="sig-line"></div><p><strong>Consultant</strong></p><p>Discharge Summary</p></div></div>';
  html += `<div class="footer"><p>This is a computer-generated summary. Please correlate clinically.</p><p>&copy; ${new Date().getFullYear()} ${escapeHtml(HOSPITAL_NAME)}</p></div>`;
