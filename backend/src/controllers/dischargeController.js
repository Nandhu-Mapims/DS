...
import { enhanceDischargeWithAI, suggestIcd10Codes } from '../services/aiService.js';
...
/**
 * POST /discharge/suggest-icd10
 * AI-powered ICD-10 suggestion from diagnosis text.
 */
export async function suggestIcd10(req, res) {
  const { provisionalDiagnosis, finalDiagnosis, clinicalDetails } = req.body;
  if (!provisionalDiagnosis && !finalDiagnosis) {
    return error(res, 'At least one diagnosis field is required', 400);
  }
  const result = await suggestIcd10Codes({ provisionalDiagnosis, finalDiagnosis, clinicalDetails });
  if (result.error && result.codes.length === 0) {
    return error(res, result.error, 422);
  }
  return success(res, result, 'ICD-10 codes suggested');
}
